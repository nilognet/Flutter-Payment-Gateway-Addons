library paystack_gateway;

import 'package:flutter/widgets.dart';
import 'package:payment_base/payment_base.dart';
import 'package:paystack_gateway/widgets/paystack_native_screen.dart';

class PayStackGateway implements PaymentBase {
  /// Payment method key
  ///
  static const key = "paystack";

  @override
  String get libraryName => "paystack_gateway";

  @override
  String get logoPath => "assets/images/pay_stack_logo.png";

  @override
  Future<void> initialized({
    required BuildContext context,
    required RouteTransitionsBuilder slideTransition,
    required Future<dynamic> Function(List<dynamic>) checkout,
    required Function(dynamic data) callback,
    required String amount,
    required String currency,
    required Map<String, dynamic> billing,
    required Map<String, dynamic> settings,
    required Future<dynamic> Function({String? cartKey, required Map<String, dynamic> data}) progressServer,
    required String cartId,
    required Widget Function(String url, BuildContext context) webViewGateway,
  }) async {
    bool testMode = settings['testmode']['value'] == "yes";
    dynamic checkoutData;
    try {
      checkoutData = await checkout([]);
    } catch (e) {
      callback(e);
      return;
    }
    int orderId = checkoutData["order_id"];
    final result = await Navigator.push(
      context,
      PageRouteBuilder(
        pageBuilder: (context, _, __) => PayStackNativeScreen(
          amount: amount,
          email: checkoutData["billing_address"]["email"],
          currency: currency,
          checkout: (data) {},
          orderId: orderId.toString(),
          orderKey: checkoutData["order_key"],
          publicKey: testMode ? settings['test_public_key']['value'] : settings['live_public_key']['value'],
          progressServer: progressServer,
          callback: callback,
          cartId: cartId,
        ),
        transitionsBuilder: slideTransition,
      ),
    );
    if (result != null) {
      callback({
        'redirect': result['redirect'],
        'order_received_url': result['order_received_url'],
      });
    }
  }

  @override
  String getErrorMessage(Map<String, dynamic>? error) {
    if (error == null) {
      return 'Something wrong in checkout!';
    }
    if (error['message'] != null) {
      return error['message'];
    }
    return 'Error!';
  }
}
